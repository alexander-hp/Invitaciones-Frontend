...
@Component({
  selector: 'app-new-public-invitation-boda-creativa-premium',
  templateUrl: './new-public-invitation-boda-creativa-premium.component.html',
  styleUrls: ['./new-public-invitation-boda-creativa-premium.component.css']
})
export class NewPublicInvitationBodaCreativaPremiumComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    try {
      this.ngZone.runOutsideAngular(() => {
        confetti({
          ...
          spread: 90,
          origin: { y: 0.6 },
          colors: ['#ffd700', '#dfba87', '#fff8e7', '#c59b6c', '#b38749']
        });
        setTimeout(() => {
          confetti({
            particleCount: 60,
  ...
  shareWhatsApp(): void {
    ...
    const title = this.invitation.content.headline || this.event?.title || 'Boda Real';
    const text = encodeURIComponent(`¡Te comparto la invitación a nuestra boda: ${title}! Mira todos los detalles aquí: ${window.location.href}`);
  ...
  get calendarUrl(): string {
    ...
    const details = this.publicInvitationUrl ? `Confirma tu asistencia: ${this.publicInvitationUrl}` : '';
  ...
  uploadPhoto(): void {
    if (this.selectedFile) {
      this.uploadPhotoOutput.emit(this.selectedFile);
